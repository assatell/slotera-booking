...
test('RC65.3.1 invalidates the cron audit throttle when registry schema changes', () => {
  ...
  assert.match(source, /AUDIT_SCHEMA_VERSION = 2/);
  assert.match(source, /\$schema === self::AUDIT_SCHEMA_VERSION/);
});

test('RC65.3.1 throttled audit self-heals missing secure cleanup and PayPal reconciliation', () => {
  const php = `
    ...
    function wp_schedule_event($ts,$schedule,$hook){ global $scheduled; $scheduled[$hook]=$schedule; return true; }
